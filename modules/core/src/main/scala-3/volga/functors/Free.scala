...
enum Free[+F[+_], +A]:
    ...
    final def foldMap[M[+x]](t: [A] => F[A] => M[A])(using M: Monad[M]): M[A] =
        import Monad.Recur
        M.recur(this) {
            case Pure(a)     => Recur.Done(a)
            case Embed(fa)   => Recur.Lift(t(fa).map(Recur.Done(_)))
            case Bind(fa, f) =>
                fa match
                    case Pure(a)     => Recur.Yield(f(a))
                    case Embed(fa)   => Recur.Lift(t(fa).map(x => Recur.Yield(f(x))))
                    case Bind(fb, g) => Recur.Yield(fb.flatMap(x => g(x).flatMap(f)))
        }
...
@main def check() =
    def go(x: Int, trace: Int = 0): Free[[a] =>> (a, a), Int] =
        ...
        else if x % 1000 == 0 then Free.Embed[[a] =>> (a, a), Int]((1, 0)).flatMap(i => go(x - 1, trace * 10 + i))
